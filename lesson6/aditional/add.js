...
document.write('[' + (cutString1('наслаждение', 3)) + ']');
...
// крапку яка знаходиться не меньше ніж на 2 символ далі після равлика, функція не чутлива до регістру (some@email.com,SOME@EMAIL.COM,some@EMAIL.com,
...
let check = (email) => {
    if (email.includes('@') && email.indexOf('@') > 2 && email.lastIndexOf('.') - email.indexOf('@') > 2) {
        return 'true';
    } else {
        return 'false';
    }
...
let sort = (arr) => {
    return arr.sort((u1, u2) => u2.modules.length - u1.modules.length);
}
...
document.writeln(cutString(str1, 5))
